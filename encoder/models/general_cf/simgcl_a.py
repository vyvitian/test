...
from models.general_cf.lightgcn_a import LightGCN_a
from models.loss_utils import cal_bpr_loss, reg_params, cal_infonce_loss

init = nn.init.xavier_uniform_
uniformInit = nn.init.uniform

class SimGCL_a(LightGCN_a):
    def __init__(self, data_handler):
        super(SimGCL_a, self).__init__(data_handler)
    ...
    def forward(self, adj=None, perturb=False):
        if adj is None:
            adj = self.adj
        if not perturb:
            return super(SimGCL_a, self).forward(adj, 1.0)
