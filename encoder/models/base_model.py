...
class BaseModel(nn.Module):
    def __init__(self, data_handler):
        ...
        self.attr_num = configs['data'].get('attr_num', 0)
        self.embeding_size = configs['model']['embedding_size']

        # hyper-parameter
        if configs['data']['name'] in configs['model']:
            self.hyper_config = configs['model'][configs['data']['name']]
            for key in self.hyper_config:
                if key in configs['model']:
                    configs['model'][key] = self.hyper_config[key]
        else:
