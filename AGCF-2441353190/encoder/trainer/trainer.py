...
class Trainer(object):
    ...
    def train_epoch(self, model, epoch_idx):
        ...
        # import pdb
        # pdb.set_trace()
        if configs.get('tqdm', False) == True:
            iterable = tqdm(enumerate(train_dataloader), desc='Training Recommender', total=len(train_dataloader))
        else:
            iterable = enumerate(train_dataloader)
        for _, tem in iterable:
...
class AutoCFTrainer(Trainer):
    ...
    def train_epoch(self, model, epoch_idx):
        ...
        # start this epoch
        model.train()
        if configs.get('tqdm', True) == True:
            iterable = tqdm(enumerate(train_dataloader), desc='Training Recommender', total=len(train_dataloader))
        else:
            iterable = enumerate(train_dataloader)

        for i, tem in iterable:
            self.optimizer.zero_grad()
            batch_data = list(map(lambda x: x.long().to(configs['device']), tem))
